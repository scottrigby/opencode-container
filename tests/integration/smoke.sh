...
run_test "unknown global option errors" "Unknown global option" "$OC" --unknown tui

# --- Fast path (no --feature-file) reaches podman check ---
# These fail at "podman: command not found" which confirms parsing succeeded
run_test "default path reaches podman" "podman: command not found" "$OC" tui
...
run_test "--feature-file reaches devcontainer up (no podman)" "devcontainer up failed" "$OC" --feature-file "$TMP_JSON" tui
rm -f "$TMP_JSON"

# --- Completion scripts contain new flags ---
...
run_test "web mode with port reaches podman" "podman: command not found" "$OC" web --port 5000
